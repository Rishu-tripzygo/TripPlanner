...
async function generateWithOpenAI(body: AITripPlannerRequest, systemPrompt: string) {
...
async function generateWithGemini(body: AITripPlannerRequest, systemPrompt: string) {
...
function getProviderOrder(): Provider[] {
  ...
  const validProviders = configuredOrder.filter(
    (value): value is Provider => value === "gemini" || value === "openai"
  );
  ...
  return (["gemini", "openai"] as Provider[]).filter((provider) => {
    if (provider === "gemini") return hasUsableGeminiKey();
    return hasUsableOpenAIKey();
  });
}

async function generateItinerary(body: AITripPlannerRequest, systemPrompt: string) {
  const errors: string[] = [];
  ...
  for (const provider of getProviderOrder()) {
    try {
      if (provider === "gemini") {
        return {
          itinerary: await generateWithGemini(body, systemPrompt),
          provider,
        };
      }

      return {
        itinerary: await generateWithOpenAI(body, systemPrompt),
        provider,
      };
    } catch (error) {
      const message =
...
export async function POST(request: Request) {
  ...
  try {
    const { itinerary, provider } = await generateItinerary(body, systemPrompt);
